...
def pre_processing_file(input_file):
    log = logger.Log.get_logger()

    log.info('Deskewing the file: {}'.format(input_file))
    _rotate_file(log, input_file)

    log.info('Remove shadow from the file: {}'.format(input_file))
    _remove_shadow(input_file)

    return True


def _remove_shadow(input_file):
...
def _rotate_file(log, input_file):
    ...
    log.info("Image {} de-skew with angle: {:.3f}".format(input_file, angle))
